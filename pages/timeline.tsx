...
const TimelinePage: NextPage = () => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'posts' }, async (payload) => {
        ...
        const { data: profile } = await supabase
          ...
          .single();
        // 新しい投稿を既存のアイテムの先頭に追加する
        // mutateの第二引数をfalseにすることで、即時UI更新後に再検証を行わない
        mutate(
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'comments' }, async (payload) => {
        ...
        mutate((currentItems) => {
          // 元の投稿をタイムラインから探す
          const parentPostIndex = (currentItems || []).findIndex(
            (item) => item.item_type === 'post' && item.id === newComment.post_id
          );
  ...
  const handlePost = async () => {
    ...
    const { error } = await supabase.from('posts').insert({ content: newPostContent, user_id: user.id });
