      ...
      {navItems.map(({ href, label, icon: Icon, showBadge }) => {
        ...
        return (
          <Link
            key={href}
            href={href}
            className={`relative flex flex-col items-center justify-center no-underline w-14 h-16 text-xs transition-all duration-300 ease-out ${
              isActive ? 'text-white' : 'text-gray-400 hover:text-white'
            }`}
          >
            <div className={`relative flex flex-col items-center transition-transform duration-300 ease-out ${isActive ? '-translate-y-1' : ''}`}>
              <Icon className="w-6 h-6 mb-1" />
              <span className={`whitespace-nowrap transition-opacity duration-300 ${isActive ? 'opacity-100' : 'opacity-70'}`}>{label}</span>
